...
func Ihu(mobile string) (int, error) {
	read := Read()
	return read, nil
	v := url.Values{}
	_now := strconv.FormatInt(time.Now().Unix(), 10)
	//fmt.Printf(_now)
	_account := "cf_ipoujda"                    //查看用户名 登录用户中心->验证码通知短信>产品总览->API接口信息->APIID
	_password := "qwdasfsfggfdsdadadmandjfsfsa" //查看密码 登录用户中心->验证码通知短信>产品总览->API接口信息->APIKEY
	_mobile := mobile
	_content := "您的验证码是：" + "red" + "。请不要把验证码泄露给其他人。"
